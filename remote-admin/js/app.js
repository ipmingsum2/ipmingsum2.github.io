...
const t = Tag.define;
const comment = t(), name = t(), typeName = t(name), propertyName = t(name), literal = t(), string = t(literal), number = t(literal), content = t(), heading$1 = t(content), keyword = t(), operator = t(), punctuation = t(), bracket = t(punctuation), meta = t();
/**
...
const paths = {
  ...
  commands: 'm13 2-9 12h7l-1 8 10-12h-7z',
  code: 'm8 5-7 7 7 7 M16 5l7 7-7 7 M14 3l-4 18',
  history: 'M3 12a9 9 0 1 0 3-6 M3 3v6h6 M12 7v5l3 2',
  ban: 'M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0 M5.6 5.6l12.8 12.8',
  shield: 'm12 2 8 4v6c0 6-8 10-8 10S4 18 4 12V6z M8 12l3 3 5-6',
  ...
  kill: 'M12 2v4 M12 18v4 M2 12h4 M18 12h4 M19 12a7 7 0 1 1-14 0 7 7 0 0 1 14 0',
  fly: 'M12 22V12 M12 12C4 12 2 6 2 2c7 0 10 6 10 10z M12 12C20 12 22 6 22 2c-7 0-10 6-10 10z',
  sound: 'M11 5 6 9H2v6h4l5 4z M15 8a6 6 0 0 1 0 8 M18 5a10 10 0 0 1 0 14',
  mute: 'M11 5 6 9H2v6h4l5 4z M16 9l6 6 M22 9l-6 6',
  shutdown: 'M12 2v10 M6 5a9 9 0 1 0 12 0',
...
const definitions = {
  ...
  speed: ['Walk speed', 'Set a player’s movement speed.', 'commands'],
...
async function api(path, options = {}) {
  ...
  if (!r.ok) {
    if (r.status === 401 && me) {
      sessionStorage.removeItem('ra-session');
      location.href = HOME + '?error=sign-in';
    }
...
const ago = (t) => {
  ...
  return d < 60
    ? 'Just now'
    : d < 3600
      ? Math.floor(d / 60) + ' min ago'
      : d < 86400
        ? Math.floor(d / 3600) + ' hr ago'
        : Math.floor(d / 86400) + ' days ago';
};
const pill = (label, tone = '') => `<span class="pill ${tone}">${esc(label)}</span>`;
const empty = (title, detail, i = 'server', compact = false) =>
  `<div class="empty ${compact ? 'compact' : ''}">${icon(i)}<strong>${esc(title)}</strong><p>${esc(detail)}</p></div>`;
...
async function refresh() {
  ...
  commands = h.commands;
  audit = h.audit;
  if (['dashboard', 'servers'].includes(view)) render();
  else if (view === 'players' && $('#player-search')) renderPlayers();
  else if (view === 'bans' && $('#ban-search')) renderBans();
  else if (view === 'history' && $('#history-search')) {
    const query = $('#history-search').value.toLowerCase();
    $('#history-table').innerHTML = historyTable(
      commands.filter((c) => [c.kind, c.target, c.actor].join(' ').toLowerCase().includes(query)),
    );
  }
}
...
function status(c) {
  ...
  if (d.some((x) => x.status === 'failed')) return ['Failed', 'red'];
  if (d.some((x) => ['partial', 'unknown', 'expired'].includes(x.status)))
    return ['Needs review', 'amber'];
  return ['Pending', 'neutral'];
}
function historyTable(list) {
  return list.length
    ? `<div class="table-wrap"><table><thead><tr><th>When</th><th>Command</th><th>Target</th><th>By</th><th>Status</th></tr></thead><tbody>${list
        .map((c) => {
          const [label, tone] = status(c);
          return `<tr><td class="subtle" title="${esc(new Date(c.created_at * 1000).toLocaleString())}">${ago(c.created_at)}</td><td><code>:${esc(c.kind)}</code>${c.deliveries.length ? `<details class="result-list"><summary>${c.deliveries.length} server result${c.deliveries.length === 1 ? '' : 's'}</summary>${c.deliveries.map((d) => `<p>${esc(d.job_id)}<br>${esc(d.status)} · ${esc(d.result || 'Waiting for server acknowledgement')}</p>`).join('')}</details>` : ''}</td><td>${esc(c.target)}</td><td class="subtle identity">${esc(c.actor.split(':').slice(1).join(':') || c.actor)}</td><td>${pill(label, tone)}</td></tr>`;
        })
        .join('')}</tbody></table></div>`
    : empty('No commands yet', 'Your commands and server results will appear here.', 'history');
}
function serverRows() {
  return servers.length
    ? servers
        ...
        .map(
          (s) =>
            `<div class="server-row"><div class="tile-icon">${icon('server')}</div><div class="server-copy"><strong>${esc(s.job_id)}</strong><small>${s.players.length} player${s.players.length === 1 ? '' : 's'} · Started ${ago(s.started_at).toLowerCase()}</small></div>${pill('Live', 'green')}</div>`,
        )
        .join('')
    : empty(
        'Waiting for your first server',
        'Publish the Roblox bridge and join the experience to connect a live server.',
        'server',
        true,
      );
}
function render() {
  ...
  if (view === 'dashboard')
    html =
      heading(
        ...
        'A clear view of your players, servers, and recent activity.',
        `<button class="button primary" data-go="commands">${icon('commands')} Run a command</button>`,
      ) +
      `<div class="stats">${[
        ['Live servers', servers.length, 'Reporting in the last 45 seconds', 'server'],
        [
          'Players online',
          servers.reduce((n, s) => n + s.players.length, 0),
          'Across connected servers',
          'players',
        ],
        ['Active bans', bans.length, 'Experience-wide restrictions', 'ban'],
        ['Recent commands', commands.length, 'Latest 100 commands', 'commands'],
      ]
        .map(
          (x) =>
            `<div class="stat"><div class="stat-top">${x[0]}${icon(x[3])}</div><div class="stat-value">${x[1]}</div><small>${x[2]}</small></div>`,
        )
        .join(
          '',
        )}</div><div class="grid-two"><section class="panel"><div class="panel-head"><div><h2>Quick commands</h2><p>A few things you might need.</p></div>${icon('commands')}</div><div class="panel-body quick-grid">${['ban', 'kick', 'fly', 'speed', 'sound', 'shutdown'].map((k) => `<button class="command-tile" data-command="${k}"><span class="tile-icon">${icon(definitions[k][2])}</span><strong>${definitions[k][0]}</strong><small>${k === 'shutdown' ? 'Migrate your players' : k === 'sound' ? 'Set the atmosphere' : k === 'speed' ? 'Change movement' : k === 'fly' ? 'Take to the skies' : k === 'kick' ? 'Remove from server' : 'Manage access'}</small></button>`).join('')}</div></section><section class="panel"><div class="panel-head"><div><h2>Live servers</h2><p>Your connected game instances.</p></div><button class="ghost" data-go="servers">View all ${icon('arrow')}</button></div>${serverRows()}<div class="panel-foot">${icon('refresh')} Server heartbeats update every 5 seconds</div></section></div><section class="panel"><div class="panel-head"><h2>Recent activity</h2><button class="ghost" data-go="history">View history ${icon('arrow')}</button></div>${historyTable(commands.slice(0, 5))}</section>`;
  if (view === 'servers')
    html =
      heading('Live servers', 'Every connected job ID, with its current player count.') +
      `<section class="panel"><div class="panel-head"><h2>Server directory</h2>${pill(servers.length + ' online', servers.length ? 'green' : 'neutral')}</div>${servers.length ? `<div class="table-wrap"><table><thead><tr><th>Job ID</th><th>Place ID</th><th>Players</th><th>Started</th><th></th></tr></thead><tbody>${servers.map((s) => `<tr><td class="mono">${esc(s.job_id)} <button class="ghost" data-copy="${esc(s.job_id)}" aria-label="Copy job ID">${icon('copy')}</button></td><td class="mono subtle">${esc(s.place_id)}</td><td>${s.players.length}</td><td class="subtle">${ago(s.started_at)}</td><td><button class="button secondary small" data-players="${esc(s.job_id)}">List players</button></td></tr>`).join('')}</tbody></table></div>` : empty('No live servers', 'Start a published server with the bridge installed to see its job ID.')}</section>`;
  if (view === 'players')
    html =
      heading('Players', 'Find players by user ID, username, or display name.') +
      `<section class="panel"><div class="panel-head"><h2>Player directory</h2><select id="player-server" class="search" aria-label="Filter by server"><option value="">All connected servers</option>${servers.map((s) => `<option value="${esc(s.job_id)}">${esc(s.job_id)}</option>`).join('')}</select><input class="search" id="player-search" placeholder="Search players…" aria-label="Search players"></div><div id="player-table"></div></section>`;
  if (view === 'commands')
    html =
      heading('Command center', 'Choose an action, select your target, and send it to the game.') +
      `<div class="command-layout"><nav class="command-menu">${Object.entries(definitions)
        .map(
          ([k, d]) =>
            `<button class="nav-item ${k === selectedCommand ? 'active' : ''}" data-command="${k}">${icon(d[2])}${d[0]}</button>`,
        )
        .join(
          '',
        )}</nav><section class="panel"><div class="panel-head"><div><h2>${definitions[selectedCommand][0]}</h2><p>${definitions[selectedCommand][1]}</p></div>${icon(definitions[selectedCommand][2])}</div><form class="panel-body" id="command-form"><div id="command-fields"></div><div class="form-bottom"><small>Actions are recorded in history with your account and server results.</small><button type="submit" class="button primary">${icon('commands')} Run command</button></div></form></section></div>`;
  if (view === 'scripts')
    html =
      heading(
        'Script editor',
  ...
  if (view === 'history')
    html =
      ...
      `<section class="panel"><div class="panel-head"><h2>Command history</h2><input class="search" id="history-search" placeholder="Search commands or targets…" aria-label="Search history"></div><div id="history-table">${historyTable(commands)}</div></section><section class="panel"><div class="panel-head"><h2>Access log</h2></div>${audit.length ? `<div class="table-wrap"><table><thead><tr><th>When</th><th>Action</th><th>By</th><th>Target</th></tr></thead><tbody>${audit.map((a) => `<tr><td>${ago(a.created_at)}</td><td>${esc(a.action)}</td><td class="identity">${esc(a.actor)}</td><td class="identity">${esc(a.target)}</td></tr>`).join('')}</tbody></table></div>` : empty('No access changes', 'Sign-ins and allowlist changes are recorded here.', 'shield', true)}</section>`;
  if (view === 'bans')
    html =
      heading(
        'Bans',
        'Manage permanent restrictions across your experience.',
        `<button class="button primary" data-command="ban">${icon('ban')} Ban player</button>`,
      ) +
  ...
  document.querySelectorAll('[data-command]').forEach(
    (b) =>
      (b.onclick = () => {
        selectedCommand = b.dataset.command;
        navigate('commands');
      }),
  );
    ...
    (b) =>
      (b.onclick = () =>
        navigator.clipboard
          ...
          .then(() => toast('Job ID copied'))
          .catch(() => toast('Clipboard access unavailable', true))),
  );
  document.querySelectorAll('[data-players]').forEach(
    (b) =>
      (b.onclick = () => {
        navigate('players');
        $('#player-server').value = b.dataset.players;
        renderPlayers();
      }),
  );
  if (view === 'players') {
    renderPlayers();
    $('#player-server').onchange = renderPlayers;
    $('#player-search').oninput = renderPlayers;
  }
  if (view === 'bans') {
    renderBans();
    $('#ban-search').oninput = renderBans;
  }
  if (view === 'history')
    $('#history-search').oninput = () => {
      const q = $('#history-search').value.toLowerCase();
      $('#history-table').innerHTML = historyTable(
        commands.filter((c) => [c.kind, c.target, c.actor].join(' ').toLowerCase().includes(q)),
      );
    };
...
function renderPlayers() {
  const q = $('#player-search').value.toLowerCase(),
    job = $('#player-server').value;
  const players = servers
    .filter((s) => !job || s.job_id === job)
    .flatMap((s) => s.players.map((p) => ({ ...p, job: s.job_id })))
    .filter((p) => [p.userId, p.username, p.displayName].join(' ').toLowerCase().includes(q));
  $('#player-table').innerHTML = players.length
    ? `<div class="table-wrap"><table><thead><tr><th>Username</th><th>Display name</th><th>User ID</th><th>Job ID</th><th></th></tr></thead><tbody>${players.map((p) => `<tr><td>${esc(p.username)}</td><td class="subtle">${esc(p.displayName)}</td><td class="mono">${esc(p.userId)}</td><td><code>${esc(p.job)}</code></td><td><button class="button secondary small" data-target="${esc(p.userId)}">Manage</button></td></tr>`).join('')}</tbody></table></div>`
    : empty('No players found', 'Choose a connected server or change your search.', 'players');
...
function renderBans() {
  const q = $('#ban-search').value.toLowerCase(),
    list = bans.filter((b) =>
...
const jobOptions = () =>
  servers
    .map(
      (s) =>
        `<option value="${esc(s.job_id)}">${esc(s.job_id)} · ${s.players.length} players</option>`,
    )
    .join('');
...
const globals = new Set(
  'game workspace script task Players Vector3 Vector2 CFrame Color3 UDim UDim2 Enum Instance BrickColor print warn pairs ipairs tonumber tostring math string table buffer coroutine require assert error pcall xpcall select unpack self'.split(
    ' ',
  ),
);
const luau = StreamLanguage.define({
  startState: () => ({ long: null, comment: false, quote: null }),
  token(stream, state) {
    if (state.long) {
      const end = ']' + state.long + ']';
      while (!stream.eol()) {
        if (stream.match(end)) {
          state.long = null;
          break;
      ...
      return state.comment ? 'comment' : 'string';
    }
    ...
    if (stream.match(/^(?:0[xX][\da-fA-F_]+|0[bB][01_]+|\d[\d_]*(?:\.[\d_]*)?(?:[eE][+-]?\d+)?)/))
      return 'number';
    if (stream.match(/^[a-zA-Z_]\w*/)) {
      const w = stream.current();
      return keywords.has(w) ? 'keyword' : globals.has(w) ? 'builtin' : 'variableName';
    }
...
function setupEditor() {
      ...
      extensions: [
        ...
        syntaxHighlighting(
          HighlightStyle.define([
            { tag: tags.keyword, color: '#c69ced' },
            { tag: tags.string, color: '#a5d78c' },
            { tag: tags.number, color: '#e8bc7a' },
            { tag: tags.comment, color: '#607594' },
            { tag: tags.standard(tags.variableName), color: '#79bfdb' },
            { tag: tags.operator, color: '#8fa9dd' },
            { tag: tags.variableName, color: '#c6d4ec' },
          ]),
        ),
        EditorView.theme(
          {
            '&': { color: '#c6d4ec' },
            '.cm-content': { caretColor: '#b8c8ff', padding: '17px 0' },
            '.cm-gutters': {
            ...
            '.cm-activeLine': { backgroundColor: '#17223b66' },
            '.cm-activeLineGutter': { backgroundColor: '#17223b66', color: '#8d9fcc' },
            '.cm-selectionBackground': { backgroundColor: '#304778!important' },
            '.cm-cursor': { borderLeftColor: '#bccaff' },
          },
          { dark: true },
        ),
  ...
  const updateTarget = () =>
    ($('#script-target-field').innerHTML = targetField($('#script-scope').value, 'script-target'));
  updateTarget();
  $('#script-scope').onchange = updateTarget;
  $('#server-mode').onclick = () => {
    mode = 'server-script';
    render();
  };
  ...
  $('#execute-script').onclick = async (e) => {
    ...
    const scope = $('#script-scope').value,
      target = $('#script-target')?.value.trim();
    if (scope !== 'global' && !target) {
      toast('Enter a target first.', true);
      return;
    }
    const source = editor.state.doc.toString();
    if (!source.trim()) {
      toast('Write a script first.', true);
      return;
    }
    if (
      await confirm(
      ...
      await send({
        ...
        scope,
        source,
        ...(scope === 'player' ? { target } : scope === 'server' ? { jobId: target } : {}),
      });
      $('#execute-script').disabled = false;
    }
  };
}
...
async function start() {
  try {
    ...
    if (ticket) {
      ...
      const exchange = await api('/api/auth/exchange', {
        method: 'POST',
        body: JSON.stringify({ ticket }),
      });
      authToken = exchange.token;
      sessionStorage.setItem('ra-session', authToken);
    }
    if (!authToken) {
      landing();
      return;
