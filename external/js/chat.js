...
function banRedirectUrl(reason) {
  const r = encodeURIComponent(reason || "No reason provided");
  return `/403?type=ban&reason=${r}`;
}
...
async function getPublicIp() {
  ...
  if (!ip) throw new Error("IP lookup returned empty response");
  return ip;
}

async function linkCurrentIpToUser(userId) {
  if (!userId) return null;
  const ip = await getPublicIp();
  const q = await sb.from("user_ip_links").upsert({
    user_id: userId,
    ip,
...
async function checkIpBanAndRedirect(ip) {
  if (!ip) return false;
  const q = await sb.from("ip_bans")
    .select("ip, reason")
    .eq("ip", ip)
    .maybeSingle();
  if (q.error) throw q.error;
  ...
  me = null;
  myProfile = null;
  location.replace(banRedirectUrl(q.data.reason));
  return true;
}

async function enforceIpBanAfterLogin(userId) {
  try {
    const ip = await linkCurrentIpToUser(userId);
    return await checkIpBanAndRedirect(ip);
  } catch (e) {
    debugLog("ipban.check", e);
    return false;
  }
}

async function adminIpBanUserByUsername(username, reason = "No reason provided") {
  await requireAdmin();
  const target = await getUserProfileByUsername(username);
  const links = await sb.from("user_ip_links").select("ip").eq("user_id", target.id);
  if (links.error) throw links.error;

  const ips = [...new Set((links.data || []).map(row => String(row.ip || "").trim()).filter(Boolean))];
  if (!ips.length) throw new Error("No linked IPs found for this user yet");
  ...
  const q = await sb.from("ip_bans").upsert(payload, { onConflict: "ip" });
  if (q.error) throw q.error;
  return { target, count: ips.length };
}

async function adminIpUnbanUserByUsername(username) {
  await requireAdmin();
  const target = await getUserProfileByUsername(username);
  const links = await sb.from("user_ip_links").select("ip").eq("user_id", target.id);
  if (links.error) throw links.error;
  const ips = [...new Set((links.data || []).map(row => String(row.ip || "").trim()).filter(Boolean))];
  if (!ips.length) return { target, count: 0 };

  const q = await sb.from("ip_bans").delete().in("ip", ips);
  if (q.error) throw q.error;
  return { target, count: ips.length };
}
...
$("btnLogin").onclick = async () => {
  ...
  try {
    ...
    if (await enforceIpBanAfterLogin(me.id)) return;
...
$("btnIpBanUser").onclick = async () => {
  try {
    await requireAdmin();
    const uname = $("targetUsername").value.trim();
    const reason = prompt("IP ban reason?", "No reason provided") || "No reason provided";
    const result = await adminIpBanUserByUsername(uname, reason);
    setAdminMsg(`IP banned ${result.count} IP(s) linked to @${result.target.username}. Reason: ${reason}`);
    toast(`IP banned ${result.count} IP(s).`, "ok");
  } catch (e) {
    setAdminMsg(errText("ip ban user", e), true);
  }
};

$("btnIpUnbanUser").onclick = async () => {
  try {
    await requireAdmin();
    const uname = $("targetUsername").value.trim();
    const result = await adminIpUnbanUserByUsername(uname);
    setAdminMsg(`Removed IP ban for ${result.count} IP(s) linked to @${result.target.username}`);
    toast(`IP unban removed ${result.count} IP(s).`, "ok");
  } catch (e) {
    setAdminMsg(errText("ip unban user", e), true);
  }
};
